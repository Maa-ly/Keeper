...
import {CrossChainArbReactive} from "../src/CrossChainArbReactive.sol";
...
contract PoolMock is IAaveV3Pool {
    ...
    function supply(address asset,uint256 amount,address onBehalfOf,uint16) external { IERC20(asset).transferFrom(msg.sender,address(this),amount); c[onBehalfOf]+=amount*1e8/1e18; }
    function withdraw(address asset,uint256 amount,address to) external returns(uint256){ IERC20(asset).transfer(to,amount); c[msg.sender]-=amount*1e8/1e18; return amount; }
    ...
    function repay(address asset,uint256 amount,uint256,address onBehalfOf) external returns(uint256){ IERC20(asset).transferFrom(msg.sender,address(this),amount); uint256 b=amount*1e8/1e18; if(d[onBehalfOf]>=b)d[onBehalfOf]-=b; else d[onBehalfOf]=0; return amount; }
...
contract CrossChainArbReactiveTest is Test {
    Mintable usdcA; Mintable usdcB; Mintable ethA; Mintable ethB; RouterMint rA; RouterMint rB; PoolMock pA; PoolMock pB; OracleMock oA; OracleMock oB; LeverageLuidation liq; CrossChainArbReactive arb;

    function setUp() public {
        usdcA=new Mintable("USDC-A","USDC-A"); usdcB=new Mintable("USDC-B","USDC-B"); ethA=new Mintable("ETH-A","ETH-A"); ethB=new Mintable("ETH-B","ETH-B"); rA=new RouterMint(); rB=new RouterMint(); pA=new PoolMock(); pB=new PoolMock(); oA=new OracleMock(); oB=new OracleMock(); liq=new LeverageLuidation(address(pA),address(rA),address(oA),address(ethA),address(usdcA)); arb=new CrossChainArbReactive(address(liq));
        arb.setChain(1,address(rA),address(pA),address(oA),address(ethA),address(usdcA),3000);
        arb.setChain(0,address(rB),address(pB),address(oB),address(ethB),address(usdcB),3000);
        usdcA.mint(address(arb),10_000e6); usdcB.mint(address(arb),10_000e6);
        arb.setParams(100, 1_000e6, 100);
        vm.prank(address(arb)); IERC20(address(usdcA)).approve(address(rA), type(uint256).max);
        vm.prank(address(arb)); IERC20(address(ethA)).approve(address(rB), type(uint256).max);
    }
    ...
    function testArbExecutesOnPriceDiff() public {
        ...
        arb.react(logA); arb.react(logB);
        uint256 balA = IERC20(address(ethA)).balanceOf(address(arb));
        uint256 balB = IERC20(address(usdcB)).balanceOf(address(arb));
        assertGt(balA,0);
        assertGt(balB,0);
    }
}
